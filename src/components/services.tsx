  ...
  return (
    ...
    >
      <style>{`
        ...
        /* Тесный экран (360×700 и подобные) — те же границы, что у блока
           «ТЕСНЫЙ ЭКРАН» в globals.css, чтобы геометрия и кегли включались
           одним и тем же условием. Круг на 5% крупнее: кегли на этом наборе
           уменьшены, текст в круге освободил место, и композиция выдерживает
           больший диаметр. Оба слагаемых min() подняты на 5%,
           чтобы прибавка не зависела от того, какое из них сработало
           (на 360×700 упирается в высоту). Призраки и пунктирное кольцо
           считаются долями диаметра — растут вместе с ним. */
        @media (max-width: 374.98px) and (max-height: 740px) {
          .svc2-mob { --svc2-mob-d: min(94.5vw, 44.1dvh); }
        }
      `}</style>
