    ...
    >
      <style>{`
        ...
        .svc2-mob {
          ...
          --svc2-mob-d: min(90vw, 42svh);  /* активный круг */
        ...
        /* Тесный экран (360×700 и подобные) — то же условие, что у блока
           ...
           Круг на 5% крупнее: кегли на этом наборе уменьшены, текст в круге
           освободил место, и композиция выдерживает больший диаметр.
           Оба слагаемых min() подняты на 5%, чтобы прибавка не зависела от
           того, какое из них сработало (на 360×700 упирается в высоту).
           Призраки и пунктирное кольцо считаются долями диаметра — растут
           вместе с ним. */
        html[data-vp="tight"] .svc2-mob { --svc2-mob-d: min(94.5vw, 44.1svh); }
        ...
        .svc2-mob-text { font-size: min(1em, calc(var(--svc2-mob-d) * 0.042)); }
      `}</style>
      ...
      >
        ...
        >
          {/* Ширина текстового блока — доля диаметра (была 62vw при круге 90vw),
              ...
              Зазор виньетка↔текст: доля диаметра, но не меньше строки текста
              (1em). Доля одна отдавала на телефоне ~10px — виньетки почти
              лежали на тексте; 1em даёт там 13–14px и воздух появляется.
              На планшете с кругом 500–660px доля крупнее 1em и выигрывает,
              поэтому вид планшета не меняется вовсе.

              t-body висит на колонке, а не на абзацах: от него берётся
              семейство, метрики и — как 1em — потолок кегля для .svc2-mob-text
              и величина зазора. */}
          <div className="t-body" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 'calc(var(--svc2-mob-d) * 0.689)', gap: 'max(1em, calc(var(--svc2-mob-d) * 0.033))' }}>
