      ...
      <style>{`
        ...
        .svc2-mob {
          /* Диаметр ограничен и шириной, и высотой. Только 90vw давало круг,
             ...
             (50svh), а круги-призраки прибиты к краям контейнера — на низком
             вьюпорте центр едет к краям и они сходятся.
             svh, а не vh и не dvh: 100vh на мобильных = «большой» вьюпорт
             (адресная строка спрятана) — расчёт промахивался бы на её высоту;
             dvh же меряет ТЕКУЩУЮ высоту, а она на Android меняется уже после
             первого кадра (панели браузера въезжают) — круг успевал
             нарисоваться крупным и на глазах ужимался. svh = «малый» вьюпорт,
             как если бы все панели показаны: всегда влезает и не дышит.

             Коэффициенты подняты сначала на 3%, затем ещё на 2% (было
             90vw / 42svh). Диаметр — единственный источник размера, поэтому
             растут разом ВСЕ круги: активный, пунктирное кольцо (× 1.0667),
             оба круга-призрака (× 0.9) и их вынос за угол (× 0.2222).

             Слагаемое по ширине упёрлось в потолок 93.7vw: кольцо — это
             диаметр × 1.0667, и уже на 93.75vw оно ровно достаёт до краёв
             экрана (обёртка .svc2-mob обрезает по overflow, так что дальше
             кольцо просто срежется по бокам и перестанет читаться кругом).
             Поэтому вторая прибавка ушла в слагаемое по высоте — на телефоне
             срабатывает именно оно. */
          --svc2-mob-d: min(93.7vw, 44.2svh);  /* активный круг */
        ...
        /* Тесный экран (360×700 и подобные) — то же условие, что у блока
           «ТЕСНЫЙ ЭКРАН» в globals.css, чтобы геометрия и кегли включались
           одним и тем же признаком: атрибут data-vp="tight" ставит на <html>
           скрипт из layout.tsx (там же — почему не @media по высоте).
           Круг на 5% крупнее базового: кегли на этом наборе уменьшены, текст
           в круге освободил место, и композиция выдерживает больший диаметр.
           Прибавка сидит на слагаемом по высоте — на тесном экране (узкий И
           низкий) всегда срабатывает именно оно: 0.454×740 = 336 меньше, чем
           0.927×375 = 348. Слагаемое по ширине оставлено как предохранитель
           на случай совсем узкого экрана и упёрто в потолок 93.5vw, при
           котором пунктирное кольцо (диаметр × 1.0667) ещё не вылезает за
           края. Призраки и кольцо считаются долями диаметра — растут вместе
           с ним. */
        html[data-vp="tight"] .svc2-mob { --svc2-mob-d: min(93.7vw, 46.3svh); }

        /* Кегль текста в круге. ИСКЛЮЧЕНИЕ из правила «размер только из
           набора t-*» (см. CLAUDE.md) — оговорённое и обоснованное:
           у этой композиции единственный источник размера — диаметр круга,
           всё остальное (кольцо, призраки, ширина текста, зазоры) считается
           от него долями. Кегль был единственным, что от диаметра НЕ зависело:
           на коротком экране круг упирается в 42svh и мельчает, а текст
           оставался прежним — 8 строк вставали в круг впритык, виньетки
           упирались в кольцо (см. скриншот на P30 Pro).

           Потолок — 1em, то есть ровно тот кегль, что даёт t-body из общего
           набора (класс висит на родительской колонке). Значит крупнее
           глобального набора текст не станет НИГДЕ: на планшете (круг
           480–660px) остаётся сегодняшние 16px, на десктопе этот блок вообще
           не показывается. Меньше — только там, где круг сам стал меньше,
           то есть на коротких экранах. Хардкода px нет, коэффициент один.

           Коэффициент 0.041, а не 0.042: диаметр подняли на 3%, и при
           неизменном коэффициенте кегль вырос бы ровно на те же 3% — вся
           композиция просто стала бы крупнее, а места тексту не прибавилось
           бы совсем. Со сниженным коэффициентом кегль остаётся прежним, а
           прибавка диаметра целиком уходит в строку и в высоту блока. */
        .svc2-mob-text { font-size: min(1em, calc(var(--svc2-mob-d) * 0.041)); }

        /* Блок со стрелками: сдвинут к левому краю на 5% ширины экрана —
           было calc(5vw + 8px), стало 8px (на телефоне 26px → 8px).
           На планшете (744–1022px) отступ прежний: там 5vw — это 37–51px,
           и сдвиг на 5% ширины прижал бы стрелки вплотную к краю. */
        .svc2-mob-nav { left: 8px; }
        @media (min-width: 744px) and (max-width: 1022px) {
          .svc2-mob-nav { left: calc(5vw + 8px); }
        }
      `}</style>
      ...
      >
        ...
        >
          {/* Ширина текстового блока — доля диаметра (была 62vw при круге 90vw),
              ...
              0.689 — это ещё и оптимум: площадь вписанного прямоугольника в
              круге максимальна около 0.7 диаметра, то есть текста влезает
              больше всего именно при этой ширине; шире — короче доступная
              высота, уже — короче строка.

              Зазор виньетка↔текст — доля диаметра (0.024). Считать его от
              строки текста (max(1em, …)) оказалось слишком много: к самому
              зазору добавляются ~4px пустого поля внутри SVG виньетки и
              половина интерлиньяжа сверху первой строки, так что видимый
              просвет получался почти вдвое больше заданного. Доля от
              диаметра держит его в пропорции круга на всех ширинах.

              t-body висит на колонке, а не на абзацах: от него берётся
              семейство, метрики и — как 1em — потолок кегля для
              .svc2-mob-text. */}
          <div className="t-body" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 'calc(var(--svc2-mob-d) * 0.689)', gap: 'calc(var(--svc2-mob-d) * 0.024)' }}>
            <Flourish w={72} color="var(--color-accent1)" />
            <div className="svc2-mob-text">
        ...
        <div className="svc2-mob-nav" style={{
          position: 'absolute',
          bottom: 'calc((50svh - var(--svc2-mob-d) / 2 + 64px) / 2)',
